...
def process(ctx, dry_run, verbose):
    ...
    with click.progressbar(pdf_files, label="Processing PDFs") as bar:
        for pdf_path in bar:
            if process_pdf(pdf_path, config, dry_run=dry_run, verbose=verbose, notify=False):
                success_count += 1
            else:
                fail_count += 1
...
def watch(ctx, verbose):
    ...
    from literature_manager.watcher import run_watch
    run_watch(ctx.obj["config"], verbose)
